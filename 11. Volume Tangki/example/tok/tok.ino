...
void loop() {
  ...
  if (millis() - trigger_cnt > 500) {
    // 01 03 01 01 00 01 D4 36  read real-time value
    // 01 03 02 00 14 B8 4B
    uint8_t tx_buf[] = { 0x01, 0x03, 0x01, 0x01, 0x00, 0x01, 0xD4, 0x36 };  //read real-time value
    Serial.write(tx_buf, sizeof(tx_buf));                                   // send read distance commond
    uint16_t len = Serial.readBytes((uint8_t*)&recv_buf, 7);
    ...
    if (len == 7 && recv_buf[1] == 0x03) {
      ...
      if (calc_crc == recv_crc) {
        uint16_t distance_val = recv_buf[3] << 8 | recv_buf[4];
        char out_txt[16] = { 0 };
        if (distance_val > 280) {
