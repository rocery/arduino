  ...
  while (true) {
    ...
    if (isButtonPressed(buttonSelect)) {
      ...
      while (isButtonPressed(buttonSelect)) {
        ...
        if (currentPressTime - buttonPressStartTime >= 50 && !buttonProcessed) {
...
void setup() {
  ...
  */
  ...
  esp_task_wdt_config_t wdtConfig = {
    ...
    .idle_core_mask = (1 << portNUM_PROCESSORS) - 1,  // Apply to all CPU cores
    .trigger_panic = true                             // ESP32 resets if timeout occurs
  };

  esp_task_wdt_init(&wdtConfig);  // Initialize watchdog
