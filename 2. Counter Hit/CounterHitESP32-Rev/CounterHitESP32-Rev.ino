...
// == Counter ==
int counter, counterAlt, newCounter, oldCounter;
...
void irSensorTask(void* pvParameters) {
  ...
  while (1) {
    ...
    if (irState == LOW && lastIRState == HIGH) {
      // Menggunakan semafor untuk mengamankan akses ke variabel counter
    ...
    lastIRState = irState;
    delay(10);  // Delay untuk menghindari pembacaan yang berlebihan
  }
}
...
void dataTask(void* pvParameters) {
  ...
  while (1) {
    ...
    */
    ...
    int tryNTP = 0;
    while (ntpStatus == false && tryNTP <= 3) {
      getLocalTime();
      tryNTP++;
      delay(50);
    }
...
void setup() {
  ...
  while (wifiMulti.run() != WL_CONNECTED && tryWiFi <= 2) {
    lcd.setCursor(0, 1);
    lcd.print("Connecting WiFi" + String(tryWiFi));
    wifiMulti.run();
    tryWiFi++;
  }
  ...
  // Buat dan jalankan tugas pada Core 0
  xTaskCreatePinnedToCore(irSensorTask, "irSensorTask", 10000, NULL, 1, NULL, 0);

  // Buat dan jalankan tugas pada Core 1
  xTaskCreatePinnedToCore(dataTask, "dataTask", 10000, NULL, 1, NULL, 1);
}
